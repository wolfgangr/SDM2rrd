...
Implementation is pure server side CGI, no JavaScript involved.  
All state management ist provided by http POST variables.  
Debugging can be activated by url included GET variable.  
Presumably jumping into preselected states by url provided variables will work, but not tested.  

### Selector
The button list at the top row is generated by configured view  list and allow **switching between a 
list of views**, each one comprise of a number of rrd plots. The currently visible view is marked boldface.  
Preselected time setting is preserved.  
URL preselection by adding `?select=viewtag` may be used by external links.  
May work for other variables as well, but not tested. Try or rtfS if in doubt.    

### Navigator
The second row of buttons and input fields allow to **browse visible plots through time** domain.  

#### Time scroll buttons
The `<< < > >>` buttons allow jumps by half / full amount of the interval currently on display.  
...
* human readable local time (time zone according to current locale)

It may serve as format template, as source to copy time and/or modify for input in the fields, and as time legend for print / screenshoots. 
Beware: visible fields are shorter than time string, be sure to avoid invisible rest sent to the script.  
Play and learn quickly.  

### Graphs
Should be visible below in the way and style configured.  
If not, a readable erros messages should be rendered, may be above the navigator section.  
Onyl in case of severe misconfiguration you may get empty pages or the infamous 
"500 Internal Server Error". If so, consult your web servers log file for reason.  
  
Grahs are stored, accessible and kept by their url in the tmp dir, as long as not overwritten by another call of the scripts.  
...
For experimenting, development and special evaluations I use [drraw](http://web.taranis.org/drraw/) .
It seems to be quite old and not maintained any more, but does some basic job.  
...
comprised of
  * proper heading
  * a table containing a [HTML form](https://github.com/wolfgangr/SDM2rrd/blob/f9ea9867b716cba20de17324005be73ec14dc6a6/render/SDMcounters.pl#L263)
  ...
  * [<img ...> tags ](https://github.com/wolfgangr/SDM2rrd/blob/f9ea9867b716cba20de17324005be73ec14dc6a6/render/SDMcounters.pl#L332)
    to display our previously generated plots
  
Some debug code is still in place but jumped over - just in case to reuse it as boilerplate for the next project...  
Some of the subs after `exit` are necessary, most are left over from debugging and earlier boilering. 
...
`rrdg_lines_ary` is a debug leftover to reuse templates form drraw, shell scripts and other sources.  
...
and returns them as array, suitable to be passed to the CGI.  
And it may serve as example for great ugly efficient PERL code ;-)  
